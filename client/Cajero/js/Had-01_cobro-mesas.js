(function () {
  ...
  function renderTableGrid() {
    ...
    var html = '';
    var takeaway = state.summary.takeaway;

    if (takeaway && takeaway.hasActiveOrders && takeaway.remainingBalance > 0) {
      html +=
        '<button type="button" data-takeaway="1" class="col-span-2 group relative overflow-hidden h-32 rounded-xl bg-primary text-on-primary flex flex-col items-center justify-center gap-2 shadow-lg transition-transform hover:scale-[1.02] active:scale-95">' +
        '<span class="material-symbols-outlined text-4xl">takeout_dining</span>' +
        '<span class="font-headline-md text-headline-md">Para Llevar</span>' +
        '<span class="font-label-lg">' + formatMoney(takeaway.remainingBalance) + '</span>' +
        '<div class="absolute top-2 right-2 bg-error text-on-error rounded-full px-2 py-0.5 text-label-md">' +
        takeaway.orderCount + ' pedido(s)</div></button>';
    }

    state.summary.tables.forEach(function (table) {
      // Las mesas fusionadas aparecen combinadas en su mesa destino ("Mesa 1 + 2").
      if (table.status === 'MERGED') return;
      var selectable = table.hasActiveOrders && table.remainingBalance > 0;
      var label = tableLabelText(table);
      html +=
        '<button type="button" data-table-id="' + table.id + '" ' +
        (selectable ? '' : 'disabled ') +
        'class="' + (selectable
          ? 'bg-tertiary-fixed hover:bg-tertiary-container transition-all cursor-pointer shadow-sm'
          : 'bg-surface-container-high border border-outline-variant opacity-60 cursor-not-allowed') +
        ' h-32 rounded-xl flex flex-col items-center justify-center gap-1 group relative">' +
    ...
    tablesGrid.querySelectorAll('[data-table-id]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openCheckout('table', parseInt(btn.getAttribute('data-table-id'), 10));
      });
    });
    var takeawayBtn = tablesGrid.querySelector('[data-takeaway]');
    if (takeawayBtn) {
      takeawayBtn.addEventListener('click', function () {
        openCheckout('takeaway', null);
      });
    }
  }
  ...
  async function loadBill() {
    ...
    var path = state.context.type === 'takeaway'
      ? '/payments/takeaway/bill'
      : '/payments/tables/' + state.context.tableId + '/bill';
    state.bill = await apiReq('GET', path);
    renderCheckout();
  }

  async function openCheckout(type, tableId) {
    state.context = { type: type, tableId: tableId };
